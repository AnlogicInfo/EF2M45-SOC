...
void test_gpio()
{
  ...
  //configure the GPIO1_0 & GPIO0_7 to output
    ...
    //write 32bit data to 256Kbit BRAM
    for(i=0; i<size_n; i++)
    {
        ELF2_Out32(ELF2_BRAM256_BaseAddr +i*4, i);
    }
    
    //read 32bit data from 256Kbit BRAM      
    for(i=0; i<size_n; i++)
    {  
        rd_value = ELF2_In32(ELF2_BRAM256_BaseAddr +i*4)        ;       
        if(i !=rd_value) 
        {   
            compare_result++;
            printf("wr_bram[%d] = %d \n\r",i,i);        
